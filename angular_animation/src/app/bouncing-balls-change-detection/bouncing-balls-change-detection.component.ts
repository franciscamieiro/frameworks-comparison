...
@Component({
  selector: 'app-bouncing-balls-change-detection',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './bouncing-balls-change-detection.component.html',
  styleUrl: './bouncing-balls-change-detection.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush
})

export class BouncingBallsChangeDetectionComponent implements OnChanges, OnDestroy {
  @Input() start = false;
  @Input() ballCount = 50;
    ...
    this.ngZone.runOutsideAngular(() => {
      const animate = (timestamp: number) => {
        ...
        if (elapsedTotal >= this.duration) {
          ...
          // ⚡ VOLTA AO ANGULAR APENAS AQUI
          this.ngZone.run(() => this.done.emit());
          return;
        }
        ...
        // atualizar bolas (SEM change detection)
